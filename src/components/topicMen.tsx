...
export default function topicMen({topic:t, close, groups}: Props){
    const {user} = useUser()
    ...
    useEffect(()=>{
        if(t && t.id && groups)
            setTopic(groups.find((it:any)=>it.id===t.id))
    },[groups,t])

    function addMemeber(){
        if(!sel.id) return
        const f = topic.follow.find((it:any)=>it.id===sel.id)
        if(f) return 
    ...
    function rmMemeber(){
        if(!selr.id) return
        const f = topic.follow.find((it:any)=>it.id===selr.id)
        if(!f || f.id===user.id) return 
    ...
    <div className="p-6 flex flex-col justify-between h-full">
        ...
        <div className="flex justify-between items-stretch h-full px-5">
